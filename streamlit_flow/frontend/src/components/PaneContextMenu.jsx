...
const CreateNodeModal = ({show, handleClose, nodes, edges, theme, setPaneContextMenu, setModalClosing, clickPosition, setNodes, handleDataReturnToStreamlit, lbData }) => {
	const visualizationMap = {};
	lbData.contents.forEach(item => {
		visualizationMap[item.visualization_name] = item.visualization_id;
	});
	...
	const onNodeVizIdChange = (e) => {
		setNewNode((newNode) => ({...newNode, nodeVizId: e.target.value}));
	}
	...
	const handleCreateNode = (e) => {
		...
		const updatedNodes = [...nodes, {
			...
			data: {content: newNode.nodeContent, vizId: newNode.nodeVizId, lbData: lbData},
	...
	visualizationMap['Hey'] = "322bd596-ea85-4468-9168-e1a735d4ffce";
	visualizationMap['Hey hey'] = "0447edee-433b-4751-ada3-0160dd715aee";
	...
	return (
		...
		<Modal.Body>
			...
			<Row className='g-2 mt-1 mt-md-0'>
				<Col md>
					<FloatingLabel controlId="floatingSelect" label="Select visualization">
						<Form.Select
							aria-label="Select visualization"
							onChange={onNodeVizIdChange}
							autoFocus
							defaultValue="" // Ensure no visualization is selected by default
						>
							<option value="" disabled>
								Select a visualization...
							</option>
							{Object.entries(visualizationMap).map(([name, id]) => (
...
const PaneConextMenu = ({paneContextMenu, setPaneContextMenu, nodes, edges, layoutOptions, setNodes, handleDataReturnToStreamlit, setLayoutCalculated, theme, lbData}) => {
